...
if __name__ == "__main__":
    ...
    iter_num = 0
    #初始化可学习的提示
    text_fea = prompt_tuning.main_worker(args,train_dataloader,clip_model_vit16,iter_num=0,max_iter=max_iter)
    ...
    while iter_num < args.FC_update_bengin:   #只更新Prompt
        ##Prompt Learning,不要动!
        print(iter_num)
        text_fea = prompt_tuning.main_worker(args,train_dataloader,clip_model_vit16,iter_num=iter_num,max_iter=max_iter)
        args.load_prompt = osp.join(args.output_dir,"prompt_model_"+str(iter_num)+".pt")   
    ...
    #我们的方法：同时更新prompt和image_encoder
    while iter_num >= args.FC_update_bengin and iter_num <= max_iter:  #更新prompt和image_encoder
        ...
        text_fea = prompt_tuning.main_worker(args,train_dataloader,clip_model_vit16,iter_num=iter_num,max_iter=max_iter)   #开始训练可学习的prompt
        args.load_prompt = osp.join(args.output_dir,"prompt_model_"+str(iter_num)+".pt")  #载入上一个iter保存的训练结果

        for name, param in clip_model_vit16.named_parameters():  #选择image_encoder可更新的层
        ...
        for i in range(len(train_dataloader)):   #更新image_encoder
