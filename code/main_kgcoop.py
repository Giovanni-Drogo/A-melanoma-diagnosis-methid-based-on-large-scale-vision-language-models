...
from promptCustom import prompt_tuning_kgcoop,clip
...
if __name__ == "__main__":
    ...
    text_fea = clip_model_vit16.encode_text(text)
    ori_text_fea = text_fea/text_fea.norm(dim=1,keepdim=True)   
    text_fea = prompt_tuning_kgcoop.main_worker(args,train_dataloader,clip_model_vit16,iter_num=0,max_iter=max_iter,ori_text_fea=ori_text_fea)
    ...
    while iter_num < args.FC_update_bengin:   #只更新Prompt
        ##Prompt Learning,不要动!
        print(iter_num)
        text_fea = prompt_tuning_kgcoop.main_worker(args,train_dataloader,clip_model_vit16,iter_num=iter_num,max_iter=max_iter,ori_text_fea=ori_text_fea)
        args.load_prompt = osp.join(args.output_dir,"prompt_model_"+str(iter_num)+".pt")   
