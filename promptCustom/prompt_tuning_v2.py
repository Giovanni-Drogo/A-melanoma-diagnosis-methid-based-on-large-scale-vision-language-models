...
def test_time_tuning(model, inputs, pesu_label,context_feature, optimizer, args):
    for j in range(args.tta_steps):
        ...
        loss_context_guided = 1-cos(text_feature_learnable,context_feature).mean()
        loss = F.cross_entropy(output,pesu_label) #+ loss_context_guided*0.2
...
def trainer(args, dataloader,model,clip_model, iter_num=0, max_iter=0):  #在main_worker内执行PromptLearner
    ...
    text_features = test_time_adapt_eval(dataloader, model,clip_model, optimizer, args)  #更新Prompt，返回更新后text_encoder输出的text_feature 

    return text_features

def test(args, dataloader,model,clip_model):  #在main_worker内执行PromptLearner
    ...
    for i in range(len(dataloader)):
        ...
        output_list.append(nn.Softmax(dim=1)(output))
...
def test_time_adapt_eval(dataloader, model,clip_model, optimizer, args):
    with torch.no_grad():
        model.train()
    ...
    for i in range(len(dataloader)):
        ...
        test_time_tuning(model,images,pesu_label,context_feature,optimizer, args)
        with torch.no_grad():
