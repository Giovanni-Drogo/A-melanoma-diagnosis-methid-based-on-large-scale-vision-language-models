...
class ClipImageEncoder(nn.Module):
    def __init__(self, device, arch="ViT-L/14", image_resolution=224, n_class=1000):
        ...
        clip, embed_dim = load(arch, device=device, download_root=DOWNLOAD_ROOT)
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        x = prompts + self.positional_embedding.type(self.dtype)  #self.positional_embedding.type(self.dtype).shape:[77,512]
...
class ClipTestTimeTuning(nn.Module):  #CustomCLIP（调用的Coop公开代码）
    ...
    def get_text_features(self):
        text_features = []
        prompts = self.prompt_learner()  #[2,77,512]
        # torch.save(prompts, 'learned_prompts.pt')
        # torch.save(prompts, 'learned_tokenized.pt')
        tokenized_prompts = self.prompt_learner.tokenized_prompts   #[2,77]
    ...
    def inference(self, image):
        ...
        text_features = self.get_text_features() #[class_num,512]
        # text_features_save = text_features.clone().detach()
        # torch.save(text_features_save, 'learned_features.pt')
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)   #[batchsize,512]     
    ...
    def inference_with_updated_logits(self, logits_new):
        ...
        return logits_new,text_features   #模型返回

    def forward(self, input):
        if isinstance(input, Tuple):
            view_0, view_1, view_2 = input
            return self.contrast_prompt_tuning(view_0, view_1, view_2)
        elif len(input.size()) == 2:  #没有实现，len()是batchsize，避免把batchsize设为2
...
def get_context_coop(clip_arch, classnames, device, n_ctx, ctx_init, learned_cls=False):
