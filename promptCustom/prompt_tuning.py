...
def test_time_tuning(model, inputs, pesu_label,context_feature, optimizer, args):
    for j in range(args.tta_steps):
        ...
        loss_context_guided = 1-cos(text_feature_learnable,context_feature).mean()  #临床提示指导
...
def main_worker(args, dataloader,clip_model, iter_num=0, max_iter=0,):  #在main_worker内执行PromptLearner
    ...
    text_features = test_time_adapt_eval(dataloader, model,clip_model, optimizer, args,iter_num)  #更新Prompt，返回更新后text_encoder输出的text_feature 
    return text_features

    

def test_time_adapt_eval(dataloader, model,clip_model, optimizer, args,iter_num):
    with torch.no_grad():
        model.train()
